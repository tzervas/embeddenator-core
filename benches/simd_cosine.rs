...
fn bench_cosine_query_workload(c: &mut Criterion) {
    ...
    group.bench_function("simd_query_10_docs", |bencher| {
        bencher.iter(|| {
            for doc in &documents {
                black_box(embeddenator::simd_cosine::cosine_simd(
                    &query,
                    black_box(doc),
                ));
            }
...
fn bench_cosine_edge_cases(c: &mut Criterion) {
    ...
    #[cfg(feature = "simd")]
    {
        group.bench_function("simd_empty_vs_empty", |bencher| {
            bencher.iter(|| {
                embeddenator::simd_cosine::cosine_simd(black_box(&empty), black_box(&empty))
            })
        });

        group.bench_function("simd_empty_vs_non_empty", |bencher| {
            bencher.iter(|| {
                embeddenator::simd_cosine::cosine_simd(black_box(&empty), black_box(&non_empty))
            })
        });
    }
