...
fn main() {
    ...
    println!(
        "Encoded pos: {:?}",
        &encoded.pos[..encoded.pos.len().min(10)]
    );
    println!(
        "Encoded neg: {:?}",
        &encoded.neg[..encoded.neg.len().min(10)]
    );
    println!(
        "Encoded pos len: {}, neg len: {}",
        encoded.pos.len(),
        encoded.neg.len()
    );
    ...
    embrfs
        .ingest_file(&test_path, "byte_0.bin".to_string(), true, &config)
        .unwrap();

    println!("Manifest files: {:?}", embrfs.manifest.files);
    println!(
        "Engram codebook keys: {:?}",
        embrfs.engram.codebook.keys().collect::<Vec<_>>()
    );

    // Check the actual vector in the codebook
    if let Some(chunk_vec) = embrfs.engram.codebook.get(&0) {
        println!(
            "Chunk 0 pos len: {}, neg len: {}",
            chunk_vec.pos.len(),
            chunk_vec.neg.len()
        );
    }
