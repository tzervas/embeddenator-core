...
fn test_reconstruction_guarantee_text() {
    ...
    embrfs
        .ingest_file(&test_path, "test.txt".to_string(), false, &config)
        .unwrap();
...
fn test_reconstruction_guarantee_binary() {
    ...
    let config = ReversibleVSAConfig::default();
    embrfs
        .ingest_file(&test_path, "binary.bin".to_string(), false, &config)
        .unwrap();
    ...
    let stats = embrfs.correction_stats();
    println!(
        "Binary file stats: {} chunks, {} perfect ({:.1}%)",
        stats.total_chunks,
...
fn test_reconstruction_guarantee_random() {
    ...
    let config = ReversibleVSAConfig::default();
    embrfs
        .ingest_file(&test_path, "random.bin".to_string(), false, &config)
        .unwrap();
    ...
    let stats = embrfs.correction_stats();
    println!(
        "Random data stats: correction overhead {:.2}%",
...
fn test_reconstruction_guarantee_empty() {
    ...
    let config = ReversibleVSAConfig::default();
    embrfs
        .ingest_file(&test_path, "empty.txt".to_string(), false, &config)
        .unwrap();
...
fn test_reconstruction_guarantee_single_byte() {
    ...
    for byte_val in 0..=255u8 {
        ...
        let config = ReversibleVSAConfig::default();
        embrfs
            .ingest_file(&test_path, format!("byte_{}.bin", byte_val), false, &config)
            .unwrap();
        ...
        assert_eq!(
            vec![byte_val],
            reconstructed,
            "Single byte {} reconstruction failed",
            byte_val
        );
...
fn test_reconstruction_guarantee_multi_file() {
    ...
    let files = vec![
        ...
        (
            "file2.txt",
            b"Another test file with more content.".to_vec(),
        ),
    ...
    for (name, _) in &files {
        let path = input_dir.join(name);
        embrfs
            .ingest_file(&path, name.to_string(), false, &config)
            .unwrap();
    }
    ...
    let stats = embrfs.correction_stats();
    println!(
        "Multi-file stats: {} total chunks, {:.1}% perfect",
        stats.total_chunks,
        stats.perfect_ratio * 100.0
    );
}
...
fn test_correction_stats_accuracy() {
    ...
    let config = ReversibleVSAConfig::default();
    embrfs
        .ingest_file(&test_path, "large.bin".to_string(), false, &config)
        .unwrap();
    ...
    assert!(
        stats.original_bytes >= large_data.len() as u64,
